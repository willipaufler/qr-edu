# Station: Extremwertprobleme

Hier findest du alle Hilfestellungen und Lösungen zu deiner Stationsarbeit. Nutze die Tipps, wenn du bei einer Aufgabe feststeckst, und die Lösungen, um dein Ergebnis zu kontrollieren.


<div class="grid cards" markdown>

<div class="neumorph-card" markdown>
**Zahlenrätsel**  
--- 
[:material-lightbulb-outline: Tipp](raetsel_t.md){.neumorph-btn}  
[:material-check-circle-outline: Lösung](raetsel_l.md){.neumorph-btn .neumorph-btn-primary}
</div>

<div class="neumorph-card" markdown>
**Rechteck unter Parabel**  
--- 
[:material-lightbulb-outline: Tipp](rechteck_t.md){.neumorph-btn}  
[:material-check-circle-outline: Lösung](rechteck_l.md){.neumorph-btn .neumorph-btn-primary}
</div>

<div class="neumorph-card" markdown>
**Rechteck mit Randwert**  
---
[:material-lightbulb-outline: Tipp](randwert_t.md){.neumorph-btn}  
[:material-check-circle-outline: Lösung](randwert_l.md){.neumorph-btn .neumorph-btn-primary}
</div>

<div class="neumorph-card" markdown>
**Dreieck und Kurvenschar**  
--- 
[:material-lightbulb-outline: Tipp](dreieck_t.md){.neumorph-btn}  
[:material-check-circle-outline: Lösung](dreieck_l.md){.neumorph-btn .neumorph-btn-primary}
</div>

<div class="neumorph-card" markdown>
**Normannisches Fenster**  
---------  
[:material-lightbulb-outline: Tipp](fenster_t.md){.neumorph-btn}  
[:material-check-circle-outline: Lösung](fenster_l.md){.neumorph-btn .neumorph-btn-primary}
</div>

<div class="neumorph-card" markdown>
**Zylinder aus Kugel**  
---
[:material-lightbulb-outline: Tipp](zylinder_t.md){.neumorph-btn}  
[:material-check-circle-outline: Lösung](zylinder_l.md){.neumorph-btn .neumorph-btn-primary}
</div>

<div class="neumorph-card" markdown>
**Kegel aus Kugel**  
---
[:material-lightbulb-outline: Tipp](kegel_t.md){.neumorph-btn}  
[:material-check-circle-outline: Lösung](kegel_l.md){.neumorph-btn .neumorph-btn-primary}
</div>

</div>